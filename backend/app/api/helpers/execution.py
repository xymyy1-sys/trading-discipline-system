...
def build_position_execution_state(
    ...
    if current_profit_pct < 0 and state == "NORMAL_HOLD":
        ...
        action = "观察但禁止加仓"
    t_forbidden = bool(hard_exit or state in {"EXIT_REQUIRED", "REDUCE_REQUIRED"} or current < structure_stop_price or (seesaw and getattr(seesaw, "risk_level", "") in {"高", "中高"}))
    ...
    volume_price_state = "跌破VWAP" if vwap and current < vwap else "VWAP上方" if vwap else "VWAP缺口"
    ...
    events = _build_events(holding, current, vwap, current_profit_pct, max_profit_pct, profit_drawdown_pct, seesaw, evidence)
    ...
    state_row.expectation_state = "EXPECTATION_INVALIDATED" if state == "EXIT_REQUIRED" else "SLIGHTLY_WEAKER" if reduce_ratio else "MATCHED"
