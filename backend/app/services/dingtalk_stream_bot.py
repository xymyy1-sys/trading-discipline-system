...
def holding_summary(holdings: list[Holding]) -> str:
    ...
    for item in holdings:
        lines.append(
            f"- **{item.name}** `{str(item.code).zfill(6)}` · "
            f"{item.position_type or '待判定'}"
        )
    lines.extend(
        [
            "",
            "金额、数量、成本和盈亏未在群消息中展示。发送“代码 + 问题”可进行证据审查。",
        ]
    )
    return "\n".join(lines)
        ...
        try:
            ...
            if holding:
                ...
                self._reply(
                    incoming,
                    f"{holding.name}持仓研判",
                    prefix + result.row.content + suffix,
                )
                return
            ...
            self._reply(
                incoming,
                "处理失败",
                f"本次问题处理失败：`{exc.__class__.__name__}`。请稍后重试。",
            )
        finally:
            db.close()
