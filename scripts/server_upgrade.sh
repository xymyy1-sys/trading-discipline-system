...
docker compose cp backend:/app/data/predeploy-backup.db "${backup_dir}/trading_discipline.db"
docker compose exec -T backend python -c 'import json,sqlite3; db=sqlite3.connect("/app/data/trading_discipline.db"); print(json.dumps({t:db.execute(f"select count(*) from {t}").fetchone()[0] for t in ("holdings","trade_logs","next_day_plans")}))' > "${backup_dir}/counts-before.json"
...
docker compose exec -T backend python -c 'import json,sqlite3; db=sqlite3.connect("/app/data/trading_discipline.db"); print(json.dumps({t:db.execute(f"select count(*) from {t}").fetchone()[0] for t in ("holdings","trade_logs","next_day_plans")}))' > "${backup_dir}/counts-after.json"
