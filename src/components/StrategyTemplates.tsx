...
export default function StrategyTemplates() {
  ...
  const save = () => {
    ...
    fetch(`${API_BASE}/api/strategies/templates/${current.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(current) })
      ...
      .then((saved: Template) => { setItems(rows => rows.map(row => row.id === saved.id ? saved : row)); setMessage(`${saved.name} 已保存为 v${saved.version}`) })
      .catch(() => setMessage('策略模板保存失败'))
  }
  return <section className="strategy-template-page">
    <header className="pos-header"><div><h2>交易剧本模板</h2><p>统一保存适用环境、前提、预期、竞价、量价、仓位、止损、持仓和禁止动作。</p></div>{message && <span className="refresh-note">{message}</span>}</header>
    ...
    {current && <article className="panel strategy-editor">
      ...
      <label className="strategy-enabled"><input type="checkbox" checked={current.enabled} onChange={e => patch({ enabled: e.target.checked })} />启用模板</label>
      <button className="grade-btn" onClick={save}><Save size={15} />保存新版本</button>
    </article>}</div>
  </section>
}
