...
describe('今日决策纪律主控条', () => {
  ...
  test('把盘后剧本到竞价验证的闭环作为今日决策主控任务展示', () => {
    ...
    expect(source).toContain('预期量价战术提醒分组')
    expect(source).toContain('严重风险')
    expect(source).toContain('禁止追高')
    expect(source).toContain('低点恐慌保护')
    expect(source).toContain('冲高兑现/利润保护')
    expect(source).toContain('planLoopTaskRisks')
    expect(source).toContain('buildPlanLoopTacticalBuckets')
    expect(source).toContain('volume_shape_guidance')
    ...
    expect(styles).toContain('.plan-loop-tactical-strip')
  })
})
